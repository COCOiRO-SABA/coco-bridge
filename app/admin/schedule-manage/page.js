...
export default function ScheduleManage() {
  ...
  const [newItem, setNewItem] = useState({ '時間帯': '', 'タイトル': '', '説明': '', '表示順': '', '公開': 'TRUE' })
  ...
  const handleAdd = async () => {
    ...
    setNewItem({ '時間帯': '', 'タイトル': '', '説明': '', '表示順': '', '公開': 'TRUE' })
    await loadData()
    setSaving(null)
  }
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#1a2744', marginBottom: '24px' }}>1日の流れ 管理</h1>
      {loading ? <p>読み込み中...</p> : (
        <>
              ...
              <div key={index} style={{ background: 'white', borderRadius: '10px', padding: '20px', marginBottom: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                    <div>
                      <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>時間帯</label>
                      <input value={item['時間帯'] || ''} onChange={e => handleChange(index, '時間帯', e.target.value)} placeholder="例: 10:00〜11:00" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>タイトル</label>
                      <input value={item['タイトル'] || ''} onChange={e => handleChange(index, 'タイトル', e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>説明</label>
                    <textarea value={item['説明'] || ''} onChange={e => handleChange(index, '説明', e.target.value)} rows={2} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', resize: 'vertical', boxSizing: 'border-box' }} />
                  </div>
          ...
          <div style={{ background: 'white', borderRadius: '10px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>時間帯</label>
                  <input value={newItem['時間帯']} onChange={e => setNewItem({ ...newItem, '時間帯': e.target.value })} placeholder="例: 10:00〜11:00" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>タイトル</label>
                  <input value={newItem['タイトル']} onChange={e => setNewItem({ ...newItem, 'タイトル': e.target.value })} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>説明</label>
                <textarea value={newItem['説明']} onChange={e => setNewItem({ ...newItem, '説明': e.target.value })} rows={2} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>表示順</label>

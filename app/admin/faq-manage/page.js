...
export default function FaqManage() {
  ...
  const [newItem, setNewItem] = useState({ '質問': '', '回答': '', '表示順': '', '公開': 'TRUE' })
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#1a2744', marginBottom: '24px' }}>よくある質問 管理</h1>
      {loading ? <p>読み込み中...</p> : (
        <>
            ...
            {items.map((item, index) => (
              <div key={index} style={{ background: 'white', borderRadius: '10px', padding: '20px', marginBottom: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div>
                    <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>質問</label>
                    <input value={item['質問'] || ''} onChange={e => handleChange(index, '質問', e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>回答</label>
                    <textarea value={item['回答'] || ''} onChange={e => handleChange(index, '回答', e.target.value)} rows={3} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', resize: 'vertical', boxSizing: 'border-box' }} />
                  </div>
          ...
          <div style={{ background: 'white', borderRadius: '10px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              <div>
                <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>質問</label>
                <input value={newItem['質問']} onChange={e => setNewItem({ ...newItem, '質問': e.target.value })} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>回答</label>
                <textarea value={newItem['回答']} onChange={e => setNewItem({ ...newItem, '回答': e.target.value })} rows={3} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>表示順</label>

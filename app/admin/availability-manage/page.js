...
export default function AvailabilityManage() {
  ...
  const [newItem, setNewItem] = useState({ '曜日': '', '午前': '', '午後': '', '備考': '', '表示順': '', '公開': 'TRUE' })
  ...
  const handleAdd = async () => {
    ...
    setNewItem({ '曜日': '', '午前': '', '午後': '', '備考': '', '表示順': '', '公開': 'TRUE' })
    await loadData()
    setSaving(null)
  }
  ...
  return (
    <AdminLayout>
      <h1 style={{ fontSize: '22px', fontWeight: '700', color: '#1a2744', marginBottom: '24px' }}>空き情報 管理</h1>
      {loading ? <p>読み込み中...</p> : (
        <>
              ...
              <div key={index} style={{ background: 'white', borderRadius: '10px', padding: '20px', marginBottom: '12px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'grid', gap: '12px' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                    <div>
                      <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>曜日</label>
                      <input value={item['曜日'] || ''} onChange={e => handleChange(index, '曜日', e.target.value)} placeholder="例: 月曜日" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>午前</label>
                      <input value={item['午前'] || ''} onChange={e => handleChange(index, '午前', e.target.value)} placeholder="例: ○ / × / △" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                    </div>
                    <div>
                      <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>午後</label>
                      <input value={item['午後'] || ''} onChange={e => handleChange(index, '午後', e.target.value)} placeholder="例: ○ / × / △" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                    </div>
                  </div>
                  <div>
                    <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>備考</label>
                    <input value={item['備考'] || ''} onChange={e => handleChange(index, '備考', e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                  </div>
          ...
          <div style={{ background: 'white', borderRadius: '10px', padding: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'grid', gap: '12px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>曜日</label>
                  <input value={newItem['曜日']} onChange={e => setNewItem({ ...newItem, '曜日': e.target.value })} placeholder="例: 月曜日" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>午前</label>
                  <input value={newItem['午前']} onChange={e => setNewItem({ ...newItem, '午前': e.target.value })} placeholder="例: ○" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                </div>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>午後</label>
                  <input value={newItem['午後']} onChange={e => setNewItem({ ...newItem, '午後': e.target.value })} placeholder="例: ○" style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>備考</label>
                <input value={newItem['備考']} onChange={e => setNewItem({ ...newItem, '備考': e.target.value })} style={{ width: '100%', padding: '8px 12px', border: '1px solid #e0e0e0', borderRadius: '6px', fontSize: '14px', boxSizing: 'border-box' }} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ fontSize: '12px', color: '#999', display: 'block', marginBottom: '4px' }}>表示順</label>
